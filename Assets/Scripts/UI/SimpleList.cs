    ...
    public class SimpleList<TView, TData> where TView : MonoBehaviour, IDataView<TData>
    {
        ...
        [SerializeField, HideInInspector] protected List<TView> instances = new List<TView>();
        [SerializeField, HideInInspector] protected List<GameObject> separators = new List<GameObject>() {null};
        ...
        public TView GetElement(int index) => instances[index];

        public IEnumerable<TView> Elements() => instances.Take(_curLength);
        ...
        public int IndexOf(TView comp) => instances.IndexOf(comp);
        public int IndexOf(GameObject go) => instances.FindIndex(x => x.gameObject == go);
    }
